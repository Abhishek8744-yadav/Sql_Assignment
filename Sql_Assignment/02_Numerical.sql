...
SELECT mod(Quantity,2) as modulus from ORDERS;

-- 8. Convert negative values to positive.
SELECT abs(Quantity) from orders;
-- There is no negative value present for example i take Quantity as negative value

-- 9. Truncate price without rounding.

-- 10. Find square root of total sales.
SELECT SQRT(SUM(UNIT_PRICE)) AS sqrt_sales FROM orders;
...
SELECT abs(sum(unit_price)-avg(unit_price)) as diff_price from orders;
...
SELECT sum(unit_price)-sum( Quantity) as new_value from ORDERS;
...
SELECT round(100.58) as round_value from dual;
